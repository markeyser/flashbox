...
def test_get_sandbox_size_filtering_and_calculation(monitor, mocker):
    ...
    # Asserts
    assert monitor.total_files == 2, (
        "Should only count file1.py and file2.py, skipping .git and .venv"
    )
    assert monitor.total_size == 400, "Should sum up size of file1.py and file2.py only"
    assert monitor.last_active_time == current_time, (
        "Since file2 was recently edited, active time should be updated to check time"
    )
